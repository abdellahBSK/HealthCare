...
let doctorId; // Pour stocker l'ID d'un médecin de test
beforeAll(async () => {
    try {
...
describe('Appointment Routes', () => {
    ...
    test('GET /api/appointments/specialists should return specialists for conditions', async () => {
        ...
        const response = await request(app)
            .get(`/api/appointments/specialists?conditions=${conditionId}`);
        expect(response.status).toBe(200);
        expect(Array.isArray(response.body)).toBe(true);
    });
    test('GET /api/appointments/doctors should return doctors by specialty', async () => {
        const response = await request(app)
            .get('/api/appointments/doctors?specialty=General%20Practitioner');
        expect(response.status).toBe(200);
        expect(Array.isArray(response.body)).toBe(true);
    });
