...
export const getHealthCategories = async (req, res) => {
    try {
...
export const getSpecialistsForConditions = async (req, res) => {
    try {
        const { conditions } = req.query;
        if (!conditions) {
            return res.status(400).json({ message: 'Conditions parameter is required' });
        }
        ...
        healthConditions.forEach(condition => {
            if (condition.specialties && condition.specialties.length > 0) {
                condition.specialties.forEach(spec => specialtyNames.add(spec));
            }
        });
        // Trouver les spécialités
        const specialties = await Specialty.find({
            name: { $in: Array.from(specialtyNames) }
        });
        // Créer un tableau de recommandations
        const recommendations = specialties.map(specialty => {
            // Calculer combien de conditions sont traitées par cette spécialité
            const matchedConditions = healthConditions.filter(condition => condition.specialties.includes(specialty.name)).length;
            return {
                _id: specialty._id,
                name: specialty.name,
        ...
        recommendations.sort((a, b) => b.matchedConditionsCount - a.matchedConditionsCount);
...
export const getDoctorsBySpecialty = async (req, res) => {
    try {
        const { specialty, sort } = req.query;
        if (!specialty) {
        ...
        const formattedDoctors = await Promise.all(doctors.map(async (doctor) => {
            ...
            return {
                ...
                experience: doctor.experience ?
                    Math.max(...doctor.experience.map(exp => exp.endYear - exp.startYear)) : 0,
        ...
        if (sort === 'rating') {
            ...
            sortedDoctors.sort((a, b) => {
                if (!a.nextAvailability)
                    return 1;
                if (!b.nextAvailability)
                    return -1;
                return new Date(a.nextAvailability.date).getTime() -
...
export const getDoctorAvailability = async (req, res) => {
    try {
        ...
        const availabilityDays = await generateAvailabilityForDateRange(doctorId, startDate, endDate, doctor.availabilitySchedule);
...
export const createAppointment = async (req, res) => {
    try {
        const { doctorId, date, startTime, endTime, conditionIds, notes } = req.body;
        // Valider les données
        ...
        const isAvailable = await checkSlotAvailability(doctorId, new Date(date), startTime, endTime);
        if (!isAvailable) {
        ...
        if (conditionIds && conditionIds.length > 0) {
            ...
            if (conditions.length > 0) {
                ...
                reason.reasonType = conditions[0].category === 'Mental Health' ? 'mental' : 'physic';
            }
        }
...
const getNextAvailability = async (doctorId) => {
    try {
        ...
        const availabilityDays = await generateAvailabilityForDateRange(doctorId, today, oneWeekLater, doctor.availabilitySchedule);
...
const generateAvailabilityForDateRange = async (doctorId, startDate, endDate, availabilitySchedule) => {
    ...
    while (currentDate < endDate) {
        ...
        const daySchedule = availabilitySchedule.find(schedule => schedule.day === dayName && schedule.isAvailable);
        if (daySchedule) {
            ...
            const availableSlots = await getAvailableSlotsForDay(doctorId, new Date(currentDate), daySchedule.startTime, daySchedule.endTime);
...
const checkSlotAvailability = async (doctorId, date, startTime, endTime) => {
    try {
        ...
        const doctor = await Doctor.findById(doctorId);
        const daySchedule = doctor.availabilitySchedule.find(schedule => schedule.day === dayName && schedule.isAvailable);
        if (!daySchedule) {
