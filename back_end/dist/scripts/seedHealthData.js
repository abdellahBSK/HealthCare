...
const seedHealthData = async () => {
    try {
        ...
        const createdConditions = await HealthCondition.insertMany(healthConditions);
        ...
        for (const condition of createdConditions) {
            for (const specialtyName of condition.specialties) {
                await Specialty.updateOne({ name: specialtyName }, { $push: { commonConditions: condition._id } });
            }
        }
