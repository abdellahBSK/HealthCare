...
export const getPatientByUserId = async (req: Request, res: Response) => {
  try {
    const patient = await patientService.findByUserIdWithUserData(req.params.userId);
    if (!patient) {
      return res.status(404).json({ message: 'Patient not found' });
    }
    res.status(200).json(patient);
...
export const updatePatientByUserId = async (req: Request, res: Response) => {
  try {
    const updatedPatient = await patientService.updatePatientByUserId(req.params.userId, req.body);
    if (!updatedPatient) {
      return res.status(404).json({ message: 'Patient not found' });
    }
    res.status(200).json(updatedPatient);
...
export const deletePatientByUserId = async (req: Request, res: Response) => {
  try {
    const deletedPatient = await patientService.deletePatientByUserId(req.params.userId);
    if (!deletedPatient) {
      return res.status(404).json({ message: 'Patient not found' });
    }
    res.status(200).json({ message: 'Patient profile deleted successfully' });
